...
### 13. スクロール表示が途中で止まる / `OSError: [Errno 121]` が途中で発生

**症状**:
- スクロール表示が途中まで動作するが、途中で止まってエラーになる
- `OSError: [Errno 121] Remote I/O error` が表示される
- `i2cdetect` ではデバイスが正常に検出される

**原因**:
- フレーム間隔（`delay`）が短すぎて、I²C 通信が追いつかない
- 連続的なデータ送信により I²C バスが過負荷になっている

**確認手順**:
1. 現在の `delay` 設定を確認
   ```python
   # scroll_oled.py の scroll() メソッド
   def scroll(self, text, speed=2, delay=0.05, ...):  # delay が 0.05 以下だと不安定
   ```

2. I²C バスの状態を確認
   ```bash
   i2cdetect -y 1
   # デバイスが検出されていれば、通信速度の問題
   ```

**解決策**:
- `delay` を **0.1秒以上** に設定する

```python
# 修正前（不安定）
scroller.scroll("テキスト", delay=0.05)

# 修正後（安定）
scroller.scroll("テキスト", delay=0.1)
```

**推奨設定**:
| 設定 | 値 | 説明 |
|------|-----|------|
| delay | 0.1秒 | 安定した I²C 通信を確保 |
| speed | 2px | 適度なスクロール速度 |

**補足**:
- Raspberry Pi 5 の I²C 実装や、配線の長さによって最適な値が異なる場合があります
- 安定性を優先する場合は `delay=0.15` も検討してください
...
- 2025-12-30: スクロール途中停止問題（I²C通信過負荷）を追加
